...
$(function(){
    ...
    $.ajax({
        type:"get",
        url:"http://wxyd.vxin365.com/weixin_test/index.php/Home/User/baby_video_port",
        success:function(data){
            data=JSON.parse(data);
            console.log(data);
            for(var j=0;j<data.length;j++){
                for(var i=0;i<data[j].image.length; i++){
                    var videoId = 'video'+data[j].v_id[i];
                    var videoUrl = data[j].v_url[i];
                    var videoSrc = data[j].image[i];
                    console.log(videoId);
